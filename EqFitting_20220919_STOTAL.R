...
ant.data.sar <- list(
  ...
  log.Sppt = log10((Sugiara$Upper_Spp_Rich + Sugiara$Lower_Spp_Rich)),
  log.Area = log10(Sugiara$Patch_Area_m2)
)
...
ant.data.int <- list(
  log.interacts = log10(Sugiara$No_Int)
)
...
param.A.LSCC <- as.data.frame(cbind(log10(rnorm(length(param.A.SAR$log.c2), 2, sd(Sugiara$Connectance/100))), log10(rnorm(length(param.A.SAR$log.c2), mean(Sugiara$Connectance/100), sd(Sugiara$Connectance/100)))))
